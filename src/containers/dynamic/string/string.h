...
struct SPECIALIZED_STRING_TYPE(TYPE)* SPECIALIZED_STRING_METHOD(TYPE, construct_at)(struct SPECIALIZED_STRING_TYPE(TYPE)* const this) {\
    ...
    static_assert(sizeof(this->dynamic_data_) / sizeof(TYPE) < MAX_VALUE(TYPE), "sizeof(" "##TYPE##" ") is higher than size of 3x pointers");\
...
void* SPECIALIZED_STRING_METHOD(TYPE, destroy_at)(struct SPECIALIZED_STRING_TYPE(TYPE)* const this) {\
    if (!this->is_stack_allocated_) {\
        NAMESPACE_MEMORY_NATIVE(free)(this->dynamic_data_.buffer_);\
    };\
    return this;\
}\
/* --- Assignment functions implementation --- */\
...
void SPECIALIZED_STRING_METHOD(TYPE, swap)(\
    ...
    uint const stack_allocated_capacity = sizeof(this->dynamic_data_) / sizeof(TYPE);\
    if (this->is_stack_allocated_ && source->is_stack_allocated_) {\
        for (size_t index = 0u; index < stack_allocated_capacity; ++index) {\
            TYPE const temporary = this->stack_data_.buffer_[index];\
            this->stack_data_.buffer_[index] = source->stack_data_.buffer_[index];\
            source->stack_data_.buffer_[index] = temporary;\
        }\
        return;\
    }\
    if (!this->is_stack_allocated_ && !source->is_stack_allocated_) {\
        {\
            TYPE* const temporary = this->dynamic_data_.buffer_;\
            this->dynamic_data_.buffer_ = source->dynamic_data_.buffer_;\
            source->dynamic_data_.buffer_ = temporary;\
        }\
        {\
            uint const temporary = this->dynamic_data_.size_;\
            this->dynamic_data_.size_ = source->dynamic_data_.size_;\
            source->dynamic_data_.size_ = temporary;\
        }\
        {\
            uint const temporary = this->dynamic_data_.capacity_;\
            this->dynamic_data_.capacity_ = source->dynamic_data_.capacity_;\
            source->dynamic_data_.capacity_ = temporary;\
        }\
        return;\
    }\
    struct SPECIALIZED_STRING_TYPE(TYPE)* const stack_allocated_string = this->is_stack_allocated_ ? this : source;\
    struct SPECIALIZED_STRING_TYPE(TYPE)* const dynamically_allocated_string = this->is_stack_allocated_ ? this : source;\
    {\
        TYPE temporary[sizeof(this->dynamic_data_) / sizeof(TYPE)];\
        for (size_t index = 0u; index < stack_allocated_capacity; ++index) {\
            temporary[index] = stack_allocated_string->stack_data_.buffer_[index];\
        }\
        stack_allocated_string->dynamic_data_.buffer_ = dynamically_allocated_string->dynamic_data_.buffer_;\
        stack_allocated_string->dynamic_data_.size_ = dynamically_allocated_string->dynamic_data_.size_;\
        stack_allocated_string->dynamic_data_.capacity_ = dynamically_allocated_string->dynamic_data_.capacity_;\
        stack_allocated_string->is_stack_allocated_ = !(dynamically_allocated_string->is_stack_allocated_ = 1u);\
        for (size_t index = 0u; index < stack_allocated_capacity; ++index) {\
            dynamically_allocated_string->stack_data_.buffer_[index] = temporary[index];\
        }\
    }\
}\
/* --- Memory managment functions implementation --- */\
...
struct SPECIALIZED_STRING_TYPE(TYPE)* SPECIALIZED_STRING_METHOD(TYPE, shrink_to_fit)(struct SPECIALIZED_STRING_TYPE(TYPE)* const this) {\
    ...
    if (string_size <= stack_allocated_string_capacity) {\
        TYPE buffer[(sizeof(this->dynamic_data_) / sizeof(TYPE))];\
    ...
    TYPE* const buffer = NAMESPACE_MEMORY_NATIVE(malloc)((string_size + 1) * sizeof(TYPE));\
    for (uint index = 0u; index < string_size; ++index) {\
        TYPE_METHOD(TYPE, construct_copy_at)(&buffer[index], SPECIALIZED_STRING_METHOD(TYPE, at)(this, index));\
    }\
...
uint SPECIALIZED_STRING_METHOD(TYPE, size)(struct SPECIALIZED_STRING_TYPE(TYPE) const* const this) {\
    if (this->is_stack_allocated_) {\
        uint const capacity = SPECIALIZED_STRING_METHOD(TYPE, capacity)(this);\
        return (capacity - (uint)(*SPECIALIZED_STRING_METHOD(TYPE, at)(this, capacity)));\
    }\
